...
function preload() {
    // گرفتن رفرنس المان‌های HTML برای آپدیت متن
    // توجه: چون p5 هنوز ستاپ نشده از داکیومنت معمولی استفاده میکنیم یا سلکتور p5

    // تصاویر را لود میکنیم
    ...
    // نکته مهم: مدل‌های ml5 سنگین هستند.
    // ما بارگذاری مدل‌ها را به setup منتقل نمی‌کنیم چون preload مطمئن‌تر است،
    // اما چون preload صفحه را فریز می‌کند، متن را قبل از شروع آپدیت می‌کنیم.
}

function setup() {
    createCanvas(windowWidth, windowHeight);

    // دسترسی به المان‌های لودینگ
    loadingScreen = select('#loading-screen');
    loadingText = select('#loading-text');

    // 1. شروع بارگذاری مدل‌ها
    if(loadingText) loadingText.html('در حال دانلود مدل‌های هوش مصنوعی...');

    // بارگذاری Body Segmentation
    ...
    // 2. تنظیمات ویدیو
    // کیفیت را برای پرفورمنس کمی پایین نگه می‌داریم، اما روی کنواس بزرگ کشیده می‌شود
    video = createCapture(VIDEO, () => {
        console.log("Video Stream Ready");
        isVideoReady = true;
        checkAllLoaded("video");
    });
...
// متغیرهای کمکی برای چک کردن وضعیت لود
let segLoaded = false;
let poseLoaded = false;

function checkAllLoaded(source) {
    if (source === "seg") segLoaded = true;
    if (source === "pose") poseLoaded = true;

    // آپدیت متن برای کاربر
    if (loadingText) {
        if (!segLoaded || !poseLoaded) {
            loadingText.html('در حال دانلود مدل‌ها... (ممکن است طول بکشد)');
        } else if (!isVideoReady) {
            loadingText.html('لطفاً دسترسی دوربین را تأیید کنید...');
        }
    }

    // اگر همه چیز آماده بود
    if (segLoaded && poseLoaded && isVideoReady) {
        startDetection();
    }
}
...
function draw() {
    // اگر مدل‌ها لود نشده‌اند، چیزی نکش (یا صفحه سیاه بماند زیر لودینگ)
    if (!isModelLoaded) {
        background(0);
        return;
    }
    ...
    if (segmentation) {
        ...
        let img = video.get();
        // اطمینان از اینکه ماسک وجود دارد
        ...
        if (poses.length > 0) {
            let pose = poses[0];
            // بررسی وجود keypoints برای جلوگیری از خطا
            if(pose.keypoints && pose.keypoints.length > 6) {
...
// توابع رسم را برای تمیزی کد جدا کردم
function drawLaggingEffect() {
...
function drawAccessory(data, drawX, drawY, currentW, currentH) {
    if (data.poses.length > 0) {
        ...
        // چک کردن وجود keypoints قبل از استفاده
        if (pose.keypoints && pose.keypoints.length > 4 &&
            pose.keypoints[3].confidence > 0.1 && pose.keypoints[4].confidence > 0.1) {
            ...
            let assetSize = earDist; // سایز اکسسوری کمی بزرگتر
